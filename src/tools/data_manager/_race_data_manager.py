...
class RaceDatamanager(DataManager):
    ...
    def get_racepp_balanced_dataset(
        self, dataset: Dict[str, pd.DataFrame], out_data_dir: str
    ) -> Dict[str, pd.DataFrame]:
        """Get balanced RACE++ dataset.
        ...
        minority_size = df_train["difficulty"].value_counts().min()
        for label in df_train["difficulty"].unique():
            balanced_df_train = pd.concat(
                [
                    balanced_df_train,
                    df_train[df_train[DIFFICULTY] == label].sample(n=minority_size),
                ],
                ignore_index=True,
            )
        ...
        for split in [TRAIN, DEV, TEST]:
            balanced_dataset[split].to_csv(
                os.path.join(out_data_dir, f"racepp_balanced_{split}.csv"), index=False
            )
